...
func TestParseShowShowInterfaceQuickJsonOutput(t *testing.T) {
	...
	for i, test := range []struct {
		input      string
		exp        *ShowInterfaceQuickResponse
		shouldFail bool
		shouldErr  bool
	}{
		{
			input: "show.interface.quick",
			exp: &ShowInterfaceQuickResponse{InsAPI: struct {
				Outputs struct {
				...
				Output ShowInterfaceQuickResponseResult "json:\"output\" xml:\"output\""
			}{Output: ShowInterfaceQuickResponseResult{Body: ShowInterfaceQuickResultBody{TableInterface: []struct {
				RowInterface []struct {
		...
		dat, err := NewShowInterfaceQuickFromBytes(content)
		//fmt.Printf("\n---\n%#v\n---\n", dat) //DEBUG
		//fmt.Printf("%#v\n", dat.Flat()) //DEBUG
		if err != nil {
			if !test.shouldErr {
...
func TestParseShowShowInterfaceQuickResultJsonOutput(t *testing.T) {
	...
	for i, test := range []struct {
		input      string
		exp        *ShowInterfaceQuickResponseResult
		shouldFail bool
		shouldErr  bool
	}{
		{
			input: "result.show.interface.quick",
			exp: &ShowInterfaceQuickResponseResult{Body: ShowInterfaceQuickResultBody{TableInterface: []struct {
				RowInterface []struct {
					Interface         string "json:\"interface\" xml:\"interface\""
					State             string "json:\"state\" xml:\"state\""
					StateRsnDesc      string "json:\"state_rsn_desc\" xml:\"state_rsn_desc\""
		...
		dat, err := NewShowInterfaceQuickResultFromBytes(content)
		//fmt.Printf("\n---\n%#v\n---\n", dat) //DEBUG
		//fmt.Printf("%#v\n", dat.Flat()) //DEBUG
		if err != nil {
			if !test.shouldErr {

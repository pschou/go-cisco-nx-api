...
func TestParseShowIsisAdjDetailJsonOutput(t *testing.T) {
	...
	for i, test := range []struct {
		input      string
		exp        *ShowIsisAdjDetailResponse
		shouldFail bool
		shouldErr  bool
	}{
		{
			input: "show.isis.2.adj.det",
			exp: &ShowIsisAdjDetailResponse{InsAPI: struct {
				Outputs struct {
				...
				Output ShowIsisAdjDetailResponseResult "json:\"output\" xml:\"output\""
			}{Output: ShowIsisAdjDetailResponseResult{Body: ShowIsisAdjDetailResultBody{TableProcessTag: []struct {
								...
								{RowProcessAdj: []struct {
									...
									{AdjSysNameOut: "n9k-reg-4", AdjSysIDOut: "N/A", AdjUsageOut: "2", AdjStateOut: "UP", AdjHoldTimeOut: 0x6c088e200, AdjIntfNameOut: "Ethernet1/21", AdjDetailSetOut: true, AdjTransitionsOut: 1, AdjFlapOut: true, AdjFlapTimeOut: 0x51b1c79cc00, AdjCktTypeOut: "L2", AdjIpv4AddrOut: "45.1.1.1", AdjIpv6AddrOut: "0::", AdjBcastOut: false, AdjBfdIpv4EstablishOut: false, AdjBfdIpv6EstablishOut: false, AdjResurrectOut: false, AdjRestartCapableOut: true, AdjRestartAckOut: false, AdjRestartModeOut: false, AdjRestartAdjSeenRaOut: false, AdjRestartAdjSeenCsnpOut: false, AdjRestartAdjSeenL1CsnpOut: false, AdjRestartAdjSeenL2CsnpOut: false, AdjRestartSuppressAdjOut: false, TableAdjSid: []struct {
										...
										{RowAdjSid: []struct {
											...
											{AdjSidValue: 16, AdjSidFFlag: false, AdjSidBFlag: false, AdjSidVFlag: true, AdjSidLFlag: true, AdjSidSFlag: false, AdjSidPFlag: false, AdjSidWeight: 1}}}}},
		...
		dat, err := NewShowIsisAdjDetailFromBytes(content)
		//fmt.Printf("\n---\n%#v\n---\n", dat) //DEBUG
		//fmt.Printf("%#v\n", dat.Flat()) //DEBUG
		if err != nil {
			if !test.shouldErr {
...
func TestParseShowIsisAdjDetailResultJsonOutput(t *testing.T) {
	...
	for i, test := range []struct {
		input      string
		exp        *ShowIsisAdjDetailResponseResult
		shouldFail bool
		shouldErr  bool
	}{
		{
			input: "result.show.isis.2.adj.det",
			exp: &ShowIsisAdjDetailResponseResult{Body: ShowIsisAdjDetailResultBody{TableProcessTag: []struct {
				RowProcessTag []struct {
					ProcessTagOut string "json:\"process-tag-out\" xml:\"process-tag-out\""
					TableVrf      []struct {
						RowVrf []struct {
		...
		dat, err := NewShowIsisAdjDetailResultFromBytes(content)
		//fmt.Printf("\n---\n%#v\n---\n", dat) //DEBUG
		//fmt.Printf("%#v\n", dat.Flat()) //DEBUG
		if err != nil {
			if !test.shouldErr {
